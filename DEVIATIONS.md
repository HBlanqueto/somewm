...
| `fx.autocolor` | Dominant-color titlebar UI: samples the client content through the compositor's `client:dominant_color()` method (a C histogram over a top-row strip or a thumb-sized step grid), derives a readable foreground via WCAG luminance, and commits the color instantly. See the module header for the sampling rationale |
